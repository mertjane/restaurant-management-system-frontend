...
* @return {Promise<BookingsResponse>} - The booking data
 ...
 * @param {Bookings} bookingData - The booking data to be created
 * @returns {Promise<Bookings>} - The created booking data
 * @throws {string} - Error message if the request fails
 */

export const createBooking = createAsyncThunk<
     Bookings,
     { restaurantId: number | null; bookingData: Bookings },
     { rejectValue: string }
>("bookings/createBooking", async ({ restaurantId, bookingData }, { rejectWithValue }) => {
     try {
          const response = await axios.post<Bookings>(
               `${import.meta.env.VITE_API_URL}/bookings/restaurant/${restaurantId}/create-new-booking`,
               bookingData,
               {
                    headers: { "Content-Type": "application/json" },
               }
