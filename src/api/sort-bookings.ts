...
export const sortByNameASC = createAsyncThunk(
     "bookings/sortByNameASC",
     async (
          { restaurantId, page, size }: { restaurantId: number; page: number; size: number },
          { rejectWithValue }
     ) => {
          try {
               const response = await axios.get<BookingsResponse>(
                    `${import.meta.env.VITE_API_URL}/bookings/restaurant/${restaurantId}?page=${page - 1}&size=${size}&sort=name,asc`
               )
               return response.data;
          } catch (error: any) {
               return rejectWithValue(error.response?.data?.message || "Failed to fetch bookings");
...
export const sortByNameDesc = createAsyncThunk(
     "bookings/sortByNameDesc",
     async (
          { restaurantId, page, size }: { restaurantId: number; page: number; size: number },
          { rejectWithValue }
     ) => {
          try {
               const response = await axios.get<BookingsResponse>(
                    `${import.meta.env.VITE_API_URL}/bookings/restaurant/${restaurantId}?page=${page - 1}&size=${size}&sort=name,desc`
               )
               return response.data;
          } catch (error: any) {
               return rejectWithValue(error.response?.data?.message || "Failed to fetch bookings");
